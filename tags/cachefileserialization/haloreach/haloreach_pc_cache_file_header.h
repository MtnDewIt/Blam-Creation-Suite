...
#pragma pack(push, 8)
		struct s_cache_file_header
		{
			...
			c_flags_no_init<haloreach::e_cache_file_header_bit, unsigned char, haloreach::k_num_cache_file_header_bits> header_flags;
			...
			haloreach::s_cache_file_shared_resource_usage shared_resource_usage;
			int8_t padding[6212]; // There is definitely more data in the padding. No clue what or where tho
			tag footer_signature;
		};
#pragma pack(pop)
		static constexpr size_t k_cache_file_header_size = sizeof(s_cache_file_header);
		static_assert(k_cache_file_header_size == 0xA000);
