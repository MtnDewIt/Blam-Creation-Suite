...
template<> void byteswap_inplace<halo3::xbox360::s_cache_file_header>(halo3::xbox360::s_cache_file_header& header)
{
...
template<> void byteswap_inplace(halo3::s_section& section)
{
	byteswap_inplace(section.count);
	byteswap_inplace(section.address);
}

template<> void byteswap_inplace(halo3::s_cache_file_tag_group& tag_group)
{
...
template<> void byteswap_inplace(halo3::s_cache_file_tag_instance& tag_instance)
{
...
template<> void byteswap_inplace(halo3::s_cache_file_global_tag_index& tag_global_instance)
{
	byteswap_inplace(tag_global_instance.group_tag);
	byteswap_inplace(tag_global_instance.tag_index);
}

template<> void byteswap_inplace(halo3::s_cache_file_tag_interop_type_fixup& tag_interop)
{
	byteswap_inplace(tag_interop.interop_address);
	byteswap_inplace(tag_interop.cache_file_interop_type);
}

template<> void byteswap_inplace(::halo3::pc::s_cache_file_tags_header& tags_header)
{
...
template<> void byteswap_inplace(::halo3::xbox360::s_cache_file_tags_header& tags_header)
{
	byteswap_inplace(tags_header.tag_groups);
	byteswap_inplace(tags_header.tag_instances);
	byteswap_inplace(tags_header.global_tag_indices);
