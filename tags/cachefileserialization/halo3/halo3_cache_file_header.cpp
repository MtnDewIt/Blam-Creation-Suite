...
template<> BCS_SHARED void byteswap_inplace(halo3::pc::s_cache_file_header& header) 
{
...
template<> BCS_SHARED void byteswap_inplace(halo3::xboxone::s_cache_file_header& header) 
{
	byteswap_inplace(header.header_signature);
	byteswap_inplace(header.version);
	byteswap_inplace(header.size);
	...
	byteswap_inplace(header.original_section_bounds);
	byteswap_inplace(header.shared_resource_usage);
	byteswap_inplace(header.tags_addrs_offset);
	byteswap_inplace(header.tags_addrs_size);
	byteswap_inplace(header.patch);
	byteswap_inplace(header.patch_size);
	byteswap_inplace(header.compressed_section_offset);
	byteswap_inplace(header.compressed_section_size);
	byteswap_inplace(header.compressed_section_codec);
	byteswap_inplace(header.padding);
	byteswap_inplace(header.footer_signature);
}
...
template<> BCS_SHARED void byteswap_inplace(halo3::xboxone::s_cache_file_tags_header& tags_header) 
{
	byteswap_inplace(tags_header.tag_groups);
	byteswap_inplace(tags_header.tag_instances);
	byteswap_inplace(tags_header.global_tag_indices);
