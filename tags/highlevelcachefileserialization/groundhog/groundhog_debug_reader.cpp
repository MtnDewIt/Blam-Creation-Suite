...
c_groundhog_debug_reader::c_groundhog_debug_reader(c_groundhog_cache_cluster& cache_cluster, c_groundhog_cache_file_reader& cache_reader) :
	...
	debug_tag_name_index_buffer()
{
	...
	//aes128_decrypt(encrypted_string_id_buffer, string_id_buffer, string_id_data_count, c_groundhog_cache_file_reader::k_string_id_encryption_key);
	...
	//aes128_decrypt(encrypted_file_table_buffer, debug_tag_name_buffer, debug_tag_name_data_size, c_groundhog_cache_file_reader::k_file_name_encryption_key);
}

c_groundhog_debug_reader::~c_groundhog_debug_reader()
{
...
BCS_RESULT c_groundhog_debug_reader::string_id_to_string(string_id stringid, const char*& string)
{
...
BCS_RESULT c_groundhog_debug_reader::string_id_to_string(uint32_t string_id_index, uint32_t string_id_namespace, uint32_t string_id_length, const char*& string)
{
...
BCS_RESULT c_groundhog_debug_reader::get_tag_filepath(uint32_t tag_index, const char*& filepath)
{
	BCS_RESULT rs = BCS_S_OK;
...
BCS_RESULT c_groundhog_debug_reader::get_group_name(tag group_tag, const char*& group_name)
{
	return BCS_S_OK;
}

BCS_RESULT c_groundhog_debug_reader::get_group_id(tag group_tag, const char*& group_id)
{
	return BCS_S_OK;
}

BCS_RESULT c_groundhog_debug_reader::string_id_to_index(string_id stringid, uint32_t& string_index)
{
...
BCS_RESULT c_groundhog_debug_reader::string_id_to_index(uint32_t string_id_index, uint32_t string_id_namespace, uint32_t string_id_length, uint32_t& string_index)
{
	...
	case _platform_type_xbox_360:
	{
		...
		if (string_id_namespace == 0 && string_id_index > (string_id_index_buffer_count - (k_engine_string_ids_total - k_engine_set_counts[0])))
		{
			return BCS_E_FAIL;
		}
