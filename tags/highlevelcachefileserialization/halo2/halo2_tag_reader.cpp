...
using namespace blofeld::halo2::pc64;

c_halo2_tag_reader::c_halo2_tag_reader(c_halo2_cache_cluster& _cache_cluster, c_halo2_cache_file_reader& _cache_reader) :
...
c_halo2_tag_reader::~c_halo2_tag_reader()
{
}

BCS_RESULT c_halo2_tag_reader::read_tag_instances()
{
	BCS_RESULT rs = BCS_S_OK;

	c_halo2_debug_reader* debug_reader;
	...
	// #TODO: Handle Alpha/Beta tags header
	::halo2::s_cache_file_tags_header tags_header = *reinterpret_cast<const ::halo2::s_cache_file_tags_header*>(tag_section_buffer.begin);
	...
	if (BCS_FAILED(rs = cache_reader.virtual_address_to_relative_offset(tags_header.tag_instances, tag_instances_relative_offset)))
	{
		return rs;
	}

	tag_instance_infos.resize(tags_header.tag_count);
	const ::halo2::s_cache_file_tag_instance* tag_instances_read_pointer = reinterpret_cast<const ::halo2::s_cache_file_tag_instance*>(tag_section_buffer.begin + tag_instances_relative_offset);
	for (uint32_t tag_index = 0; tag_index < tags_header.tag_count; tag_index++)
	{
		s_halo2_tag_instance_info& tag_instance_info = tag_instance_infos[tag_index];
		::halo2::s_cache_file_tag_instance& tag_instance = tag_instance_info.instance = tag_instances_read_pointer[tag_index];
		...
		// #TODO: Handle Gen 2 Resource Data
...
BCS_RESULT c_halo2_tag_reader::init_tag_groups()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	do
	{
		...
		for (t_tag_group_iterator tag_group_iterator = blofeld_tag_groups; *tag_group_iterator; tag_group_iterator++)
		{
			...
			tag_group = new() c_halo2_tag_group(cache_cluster, blofeld_tag_group, parent_tag_group);
...
BCS_RESULT c_halo2_tag_reader::init_tag_instances()
{
	BCS_RESULT rs = BCS_S_OK;

	for (s_halo2_tag_instance_info& tag_instance_info : tag_instance_infos)
	{
		// skip null tags
...
BCS_RESULT c_halo2_tag_reader::get_tag_group_by_group_tag(tag in_group_tag, c_halo2_tag_group*& out_tag_group) const
{
	for (c_halo2_tag_group* tag_group : tag_groups)
	{
		tag group_tag;
...
BCS_RESULT c_halo2_tag_reader::get_tag_group_by_blofeld_tag_group(const s_tag_group& in_blofeld_tag_group, c_halo2_tag_group*& out_tag_group) const
{
	for (c_halo2_tag_group* tag_group : tag_groups)
	{
...
BCS_RESULT c_halo2_tag_reader::get_instance_info_by_tag_index(uint32_t tag_index, const s_halo2_tag_instance_info*& instance_info)
{
...
BCS_RESULT c_halo2_tag_reader::page_offset_to_pointer(int32_t page_offset, const void*& data)
{
	BCS_RESULT rs = BCS_S_OK;
...
BCS_RESULT c_halo2_tag_reader::get_tag_groups(c_halo2_tag_group**& out_tag_groups, uint32_t& out_tag_group_count)
{
...
BCS_RESULT c_halo2_tag_reader::get_tag_groups(c_tag_group**& out_tag_groups, uint32_t& out_tag_group_count)
{
...
BCS_RESULT c_halo2_tag_reader::get_tag_instances(c_halo2_tag_instance**& out_tag_instances, uint32_t& out_tag_instance_count)
{
...
BCS_RESULT c_halo2_tag_reader::get_tag_instances(c_tag_instance**& out_tag_instances, uint32_t& out_tag_instance_count)
{
...
BCS_RESULT c_halo2_tag_reader::get_tag_instance_by_cache_file_tag_index(uint32_t cache_file_tag_index, c_tag_instance*& out_tag_instance)
{
	BCS_RESULT rs = BCS_S_OK;

	c_halo2_tag_instance** tag_instances;
	...
	{
		c_halo2_tag_instance& tag_instance = *tag_instances[tag_instance_index];
