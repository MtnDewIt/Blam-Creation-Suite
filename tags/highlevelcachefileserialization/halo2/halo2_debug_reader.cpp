...
c_halo2_debug_reader::c_halo2_debug_reader(c_halo2_cache_cluster& cache_cluster, c_halo2_cache_file_reader& cache_reader) :
	...
	file_table_index_buffer()
{
	...
	//halo2::xbox360::s_cache_file_header& cache_file_header = cache_reader.cache_file_header;
	...
	////string_id_index_buffer = static_cast<uint32_t*>(_aligned_malloc(cache_file_header.string_id_string_storage_size, sizeof(unsigned long)));
	...
	//halo2_aes128_decrypt(encrypted_string_id_buffer, string_id_buffer, cache_file_header.string_id_string_storage_size, c_halo2_cache_file_reader::k_string_id_encryption_key);
	...
	//halo2_aes128_decrypt(encrypted_file_table_buffer, file_table_buffer, cache_file_header.file_table_length, c_halo2_cache_file_reader::k_file_name_encryption_key);
...
c_halo2_debug_reader::~c_halo2_debug_reader()
{
...
BCS_RESULT c_halo2_debug_reader::string_id_to_string(string_id stringid, const char*& string)
{
...
BCS_RESULT c_halo2_debug_reader::string_id_to_string(uint32_t string_id_index, uint32_t string_id_namespace, uint32_t string_id_length, const char*& string)
{
...
BCS_RESULT c_halo2_debug_reader::get_tag_filepath(uint32_t tag_index, const char*& filepath)
{
	BCS_RESULT rs = BCS_S_OK;
	...
	const halo2::s_cache_file_tag_instance* tag_instances_read_pointer = reinterpret_cast<const halo2::s_cache_file_tag_instance*>(buffer_info.begin + tag_instances_relative_offset);

	const halo2::s_cache_file_tag_instance& tag_instance = tag_instances_read_pointer[tag_index];

	// #TODO: Handle Gen 2 Tag Names
...
BCS_RESULT c_halo2_debug_reader::get_group_name(tag group_tag, const char*& group_name)
{
	return BCS_S_OK;
}

BCS_RESULT c_halo2_debug_reader::get_group_id(tag group_tag, const char*& group_id)
{
	return BCS_S_OK;
}

BCS_RESULT c_halo2_debug_reader::string_id_to_index(string_id stringid, uint32_t& string_index)
{
...
BCS_RESULT c_halo2_debug_reader::string_id_to_index(uint32_t string_id_index, uint32_t string_id_namespace, uint32_t string_id_length, uint32_t& string_index)
{
