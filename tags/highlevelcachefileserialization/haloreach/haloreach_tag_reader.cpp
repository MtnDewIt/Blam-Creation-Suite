...
using namespace blofeld::haloreach::pc64;

c_haloreach_tag_reader::c_haloreach_tag_reader(c_haloreach_cache_cluster& _cache_cluster, c_haloreach_cache_file_reader& _cache_reader) :
	...
	_interop_type_index_to_haloreach_interop_type(nullptr),
...
c_haloreach_tag_reader::~c_haloreach_tag_reader()
{
	delete _interop_type_index_to_haloreach_interop_type;
	delete interop_containers;
	delete _shared_file_index_to_cache_file_reader;
}

BCS_RESULT c_haloreach_tag_reader::read_tags_header()
{
	...
	case _platform_type_pc_64bit:
		pc_tags_header = *reinterpret_cast<const ::haloreach::pc::s_cache_file_tags_header*>(tag_section_buffer.begin + tags_header_relative_offset);
	...
	case _platform_type_xbox_360:
		xbox360_tags_header = *reinterpret_cast<const ::haloreach::xbox360::s_cache_file_tags_header*>(tag_section_buffer.begin + tags_header_relative_offset);
...
BCS_RESULT c_haloreach_tag_reader::get_tag_groups_section(s_haloreach_section& tag_groups)
{
	switch (cache_cluster.engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
...
BCS_RESULT c_haloreach_tag_reader::get_tag_instances_section(s_haloreach_section& tag_instances)
{
	switch (cache_cluster.engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
...
BCS_RESULT c_haloreach_tag_reader::get_global_tag_instances_section(s_haloreach_section& global_tag_instances)
{
	switch (cache_cluster.engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
...
BCS_RESULT c_haloreach_tag_reader::get_tag_interop_table_section(s_haloreach_section& tag_interop_table)
{
	switch (cache_cluster.engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
...
BCS_RESULT c_haloreach_tag_reader::read_tag_groups()
{
	BCS_RESULT rs = BCS_S_OK;

	c_haloreach_debug_reader* debug_reader;
	...
	s_haloreach_section tag_groups_section;
	...
	const ::haloreach::s_cache_file_tag_group* tag_groups_read_pointer = reinterpret_cast<const ::haloreach::s_cache_file_tag_group*>(tag_section_buffer.begin + tag_groups_relative_offset);
	for (uint32_t group_index = 0; group_index < tag_groups_section.count; group_index++)
	{
		s_haloreach_tag_group_info& tag_group_info = tag_group_infos[group_index];
		::haloreach::s_cache_file_tag_group& tag_group = tag_group_info.group = tag_groups_read_pointer[group_index];
...
BCS_RESULT c_haloreach_tag_reader::read_tag_instances()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	s_haloreach_section tag_instances_section;
	...
	const ::haloreach::s_cache_file_tag_instance* tag_instances_read_pointer = reinterpret_cast<const ::haloreach::s_cache_file_tag_instance*>(tag_section_buffer.begin + tag_instances_relative_offset);
	for (uint32_t tag_index = 0; tag_index < tag_instances_section.count; tag_index++)
	{
		s_haloreach_tag_instance_info& tag_instance_info = tag_instance_infos[tag_index];
		::haloreach::s_cache_file_tag_instance& tag_instance = tag_instance_info.instance = tag_instances_read_pointer[tag_index];
		// #TODO: Handle big endian 
...
BCS_RESULT c_haloreach_tag_reader::read_tag_global_instances()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	s_haloreach_section global_tag_instances_section;
	...
	const ::haloreach::s_cache_file_global_tag_index* global_tag_instances_read_pointer = reinterpret_cast<const ::haloreach::s_cache_file_global_tag_index*>(tag_section_buffer.begin + tag_global_instances_relative_offset);
	for (uint32_t global_tag_index = 0; global_tag_index < global_tag_instances_section.count; global_tag_index++)
	{
		s_haloreach_tag_global_instance_info& global_instance_info = tag_global_instance_infos[global_tag_index];
		::haloreach::s_cache_file_global_tag_index& global_tag_instance = global_instance_info.global_instance = global_tag_instances_read_pointer[global_tag_index];
		// #TODO: Handle big endian 
...
BCS_RESULT c_haloreach_tag_reader::read_tag_interops()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	s_haloreach_section tag_interop_table_section;
	...
	const s_haloreach_tag_interop_type_fixup* tag_interops_read_pointer = reinterpret_cast<const s_haloreach_tag_interop_type_fixup*>(tag_section_buffer.begin + tag_interops_relative_offset);
	for (uint32_t interop_index = 0; interop_index < tag_interop_table_section.count; interop_index++)
	{
		s_haloreach_tag_interop_type_fixup& tag_interop = tag_interop_infos[interop_index] = tag_interops_read_pointer[interop_index];
		// #TODO: Handle big endian 
...
BCS_RESULT c_haloreach_tag_reader::init_tag_groups()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	do
	{
		...
		for (t_tag_group_collection tag_group_iterator = blofeld_tag_groups; *tag_group_iterator; tag_group_iterator++)
		{
			...
			tag_group = new() c_haloreach_tag_group(cache_cluster, blofeld_tag_group, parent_tag_group);
			tag_groups.push_back(tag_group);

			s_haloreach_tag_group_info* tag_group_info = nullptr;
...
BCS_RESULT c_haloreach_tag_reader::init_tag_instances()
{
	BCS_RESULT rs = BCS_S_OK;

	for (s_haloreach_tag_instance_info& tag_instance_info : tag_instance_infos)
	{
		if (tag_instance_info.identifier != USHRT_MAX)
		{
				...
				tag_instance_info.group_info->group.group_tag == SOUND_CACHE_FILE_GESTALT_TAG) // #TODO: only sound tags for now from 360, tag definitions need fixing
			{
				...
				c_haloreach_tag_instance* tag_instance = new() c_haloreach_tag_instance(
					cache_cluster,
					*this,
...
BCS_RESULT c_haloreach_tag_reader::get_tag_group_by_group_tag(tag in_group_tag, c_haloreach_tag_group*& out_tag_group) const
{
	for (c_haloreach_tag_group* tag_group : tag_groups)
	{
		tag group_tag;
...
BCS_RESULT c_haloreach_tag_reader::get_tag_group_by_blofeld_tag_group(const s_tag_group& in_blofeld_tag_group, c_haloreach_tag_group*& out_tag_group) const
{
	for (c_haloreach_tag_group* tag_group : tag_groups)
	{
		s_tag_group const* blofeld_tag_group;
...
BCS_RESULT c_haloreach_tag_reader::get_tag_group_info_by_group_tag(tag group_tag, s_haloreach_tag_group_info*& out_tag_group_info)
{
	for (s_haloreach_tag_group_info& tag_group_info : tag_group_infos)
	{
...
BCS_RESULT c_haloreach_tag_reader::get_tag_group_info_by_blofeld_tag_group(const s_tag_group& blofeld_tag_group, s_haloreach_tag_group_info*& out_tag_group_info)
{
	for (s_haloreach_tag_group_info& tag_group_info : tag_group_infos)
	{
...
BCS_RESULT c_haloreach_tag_reader::get_global_instance_info(tag group_tag, const s_haloreach_tag_global_instance_info*& out_global_instance_info)
{
	for (s_haloreach_tag_global_instance_info& global_instance_info : tag_global_instance_infos)
	{
...
BCS_RESULT c_haloreach_tag_reader::get_instance_info_by_tag_index(uint32_t tag_index, const s_haloreach_tag_instance_info*& instance_info)
{
...
BCS_RESULT c_haloreach_tag_reader::get_tag_instance_info_by_tag_index(uint32_t tag_index, c_haloreach_tag_instance*& tag_instance)
{
...
BCS_RESULT c_haloreach_tag_reader::page_offset_to_pointer(int32_t page_offset, const void*& data)
{
	BCS_RESULT rs = BCS_S_OK;
...
BCS_RESULT c_haloreach_tag_reader::get_tag_groups(c_haloreach_tag_group**& out_tag_groups, uint32_t& out_tag_group_count)
{
...
BCS_RESULT c_haloreach_tag_reader::get_tag_groups(c_tag_group**& out_tag_groups, uint32_t& out_tag_group_count)
{
...
BCS_RESULT c_haloreach_tag_reader::get_tag_instances(c_haloreach_tag_instance**& out_tag_instances, uint32_t& out_tag_instance_count)
{
...
BCS_RESULT c_haloreach_tag_reader::get_tag_instances(c_tag_instance**& out_tag_instances, uint32_t& out_tag_instance_count)
{
...
BCS_RESULT c_haloreach_tag_reader::get_tag_instance_by_cache_file_tag_index(uint32_t cache_file_tag_index, c_tag_instance*& out_tag_instance)
{
	BCS_RESULT rs = BCS_S_OK;

	c_haloreach_tag_instance** tag_instances;
	...
	{
		c_haloreach_tag_instance& tag_instance = *tag_instances[tag_instance_index];
...
BCS_RESULT c_haloreach_tag_reader::init_interop_table()
{
	BCS_RESULT rs = BCS_S_OK;

	// #TODO: Fix once we have valid tag definitions
	/*

	const s_haloreach_tag_global_instance_info* global_instance_info;
	...
	//_resource_type_index_to_haloreach_resource_type = new() e_haloreach_resource_type[resource_gestalt.resource_type_identifiers_block.count];
	...
	//	e_haloreach_resource_type resource_type = k_num_haloreach_resource_types;
	//	for (uint32_t resource_type_index = 0; resource_type_index < k_num_haloreach_resource_types; resource_type_index++)
	//	{
	//		const s_haloreach_tag_resource_info& resource_info = haloreach_tag_resource_infos[resource_type_index];
	...
	//			)
	//		{
	//			resource_type = static_cast<e_haloreach_resource_type>(resource_type_index);
	//			break;
	//		}
	//	}
	//	ASSERT(resource_type != k_num_haloreach_resource_types);

	//	_resource_type_index_to_haloreach_resource_type[index] = resource_type;
...
BCS_RESULT c_haloreach_tag_reader::init_interops()
{
	...
	interop_containers = new() c_haloreach_interop_container * [interop_count + 1];
	interop_containers[interop_count] = nullptr;
	for (uint32_t interop_index = 0; interop_index < interop_count; interop_index++)
	{
		s_haloreach_tag_interop_type_fixup& cache_file_tag_interop_entry = tag_interop_infos[interop_index];

		e_haloreach_interop_type interop_type;
		if (BCS_FAILED(rs = interop_type_index_to_haloreach_interop_type(cache_file_tag_interop_entry.cache_file_interop_type, interop_type)))
		{
			return rs;
		}
		...
		switch (interop_type)
		{
		case _haloreach_vectorart_vertex_buffer_interop:
			interop_containers[interop_index] = new() c_haloreach_vectorart_vertex_buffer_interop(*this, tag_interop.descriptor, interop_data);
			break;
		case _haloreach_polyart_index_buffer_interop:
			interop_containers[interop_index] = new() c_haloreach_polyart_index_buffer_interop(*this, tag_interop.descriptor, interop_data);
			break;
		case _haloreach_polyart_vertex_buffer_interop:
			interop_containers[interop_index] = new() c_haloreach_polyart_vertex_buffer_interop(*this, tag_interop.descriptor, interop_data);
			break;
		case _haloreach_d3d_vertex_buffer_interop:
		case _haloreach_d3d_index_buffer_interop:
		case _haloreach_d3d_texture_interop:
		case _haloreach_d3d_texture_interleaved_interop:
		case _haloreach_d3d_vertex_shader_interop:
		case _haloreach_d3d_pixel_shader_interop:
		default:
...
BCS_RESULT c_haloreach_tag_reader::interop_type_index_to_haloreach_interop_type(int32_t type_index, e_haloreach_interop_type& interop_type)
{
	...
	interop_type = _interop_type_index_to_haloreach_interop_type[type_index];
...
BCS_RESULT c_haloreach_tag_reader::init_resource_table()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	_interop_type_index_to_haloreach_interop_type = new() e_haloreach_interop_type[resource_gestalt.interop_type_identifiers_block.count];
	for (uint32_t index = 0; index < resource_gestalt.interop_type_identifiers_block.count; index++)
	{
		...
		e_haloreach_interop_type interop_type = k_num_haloreach_interop_types;
		for (uint32_t interop_type_index = 0; interop_type_index < k_num_haloreach_interop_types; interop_type_index++)
		{
			const s_haloreach_tag_interop_info& interop_info = haloreach_tag_interop_infos[interop_type_index];
			...
			{
				interop_type = static_cast<e_haloreach_interop_type>(interop_type_index);
				break;
			}
		}
		ASSERT(interop_type != k_num_haloreach_interop_types);

		_interop_type_index_to_haloreach_interop_type[index] = interop_type;
...
class c_haloreach_pageable_streaming_resource :
	public h_resource
{
public:
	...
	c_haloreach_pageable_streaming_resource(
	...
	BCS_RESULT load_data()
	{
		...
		if (default_location.required_location >= 0)
		{
			...
			c_haloreach_cache_file_reader* resource_cache_file = new() c_haloreach_cache_file_reader(required_cache_file_path, engine_platform_build);
		...
		if (default_location.optional_location >= 0)
		{
			...
			c_haloreach_cache_file_reader* resource_cache_file = new() c_haloreach_cache_file_reader(optional_cache_file_path, engine_platform_build);
	...
	{
		return "c_haloreach_pageable_streaming_resource";
	}
};

BCS_RESULT c_haloreach_tag_reader::init_resources()
{
	BCS_RESULT rs = BCS_S_OK;
	...
	const s_haloreach_tag_global_instance_info* resource_gestalt_global_instance_info;
	...
	const s_haloreach_tag_global_instance_info* resource_layout_table_global_instance_info;
	...
	for (uint32_t resource_index = 0; resource_index < resource_gestalt.resources_block.count; resource_index++)
	{
		...
		{
			c_haloreach_tag_instance* tag_instance;
			...
			if (default_location.required_location >= 0)
			{
				...
				c_haloreach_cache_file_reader* required_resource_cache_file;
				...
				if (default_location.optional_location >= 0)
				{
					...
					c_haloreach_cache_file_reader* optional_resource_cache_file;
					...
					c_haloreach_pageable_streaming_resource* pageable_streaming_resource = new() c_haloreach_pageable_streaming_resource(
...
BCS_RESULT c_haloreach_tag_reader::init_shared_files_table()
{
	...
	_shared_file_index_to_cache_file_reader = new() c_haloreach_cache_file_reader * [shared_files_block.count];
	...
	for (uint32_t shared_file_index = 0; shared_file_index < shared_files_block.count; shared_file_index++)
	{
		...
		c_haloreach_cache_file_reader* cache_file_reader = nullptr;
...
BCS_RESULT c_haloreach_tag_reader::shared_file_index_to_cache_file_reader(int32_t shared_file_index, c_haloreach_cache_file_reader*& cache_file_reader)
{
...
BCS_RESULT c_haloreach_tag_reader::get_interop_container_by_type_and_descriptor(e_haloreach_interop_type interop_type, uint32_t descriptor, c_haloreach_interop_container*& interop_container)
{

	for (c_haloreach_interop_container** interop_container_iter = interop_containers; *interop_container_iter; interop_container_iter++)
	{
...
BCS_RESULT c_haloreach_tag_reader::get_compression_codec_by_index(int32_t codec_index, e_haloreach_compression_codec& compression_codec)
{
	BCS_RESULT rs = BCS_S_OK;
	...
	if (codec_index >= 0)
	{
		...
		s_tag_persistent_identifier identifier =
		{
		...
		if (identifier == k_haloreach_xdkcompress_codec_identifier)
		{
			compression_codec = _haloreach_compression_xdkcompress;
		}
		else
		{
