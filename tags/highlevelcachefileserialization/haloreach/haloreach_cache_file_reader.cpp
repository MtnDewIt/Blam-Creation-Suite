...
c_haloreach_cache_file_reader::c_haloreach_cache_file_reader(const wchar_t* filepath, s_engine_platform_build engine_platform_build) :
	filepath(filepath),
	...
	cache_file_header()
{
	...
	{
		pc_cache_file_header = *reinterpret_cast<haloreach::pc::s_cache_file_header*>(file_info.file_view_begin);
	}
	break;
	case _platform_type_xbox_360:
	{
		xbox360_cache_file_header = *reinterpret_cast<haloreach::xbox360::s_cache_file_header*>(file_info.file_view_begin);
...
c_haloreach_cache_file_reader::~c_haloreach_cache_file_reader()
{
	ASSERT_NO_THROW(BCS_SUCCEEDED(destroy_memory_mapped_file(file)));
}

BCS_RESULT c_haloreach_cache_file_reader::get_build_info(s_cache_file_build_info& build_info) const
{
	...
	case _platform_type_pc_64bit:
	{
		...
		if (BCS_FAILED(rs = haloreach::haloreach_scenario_type_to_base_scenario_type(pc_cache_file_header.scenario_type, build_info.scenario_type)))
		{
			return rs;
		}
		if (BCS_FAILED(rs = haloreach::haloreach_cache_file_shared_file_type_to_base_cache_file_shared_file_type(pc_cache_file_header.shared_cache_file_type, build_info.shared_cache_file_type)))
		{
			return rs;
		}
		if (BCS_FAILED(rs = haloreach::haloreach_cache_file_header_flags_to_base_cache_file_header_flags(pc_cache_file_header.header_flags, build_info.header_flags)))
		{
			return rs;
		}
	...
	case _platform_type_xbox_360:
	{
		...
		if (BCS_FAILED(rs = haloreach::haloreach_scenario_type_to_base_scenario_type(xbox360_cache_file_header.scenario_type, build_info.scenario_type)))
		{
			return rs;
		}
		if (BCS_FAILED(rs = haloreach::haloreach_cache_file_shared_file_type_to_base_cache_file_shared_file_type(xbox360_cache_file_header.shared_cache_file_type, build_info.shared_cache_file_type)))
		{
			return rs;
		}
		if (BCS_FAILED(rs = haloreach::haloreach_cache_file_header_flags_to_base_cache_file_header_flags(xbox360_cache_file_header.header_flags, build_info.header_flags)))
		{
			return rs;
		}
		if (BCS_FAILED(rs = haloreach::haloreach_cache_file_shared_file_flags_to_base_cache_file_shared_file_flags(xbox360_cache_file_header.shared_map_usage, build_info.shared_map_usage)))
		{
			return rs;
		}
...
BCS_RESULT c_haloreach_cache_file_reader::get_debug_info(s_cache_file_debug_info& debug_info) const
{

	return BCS_S_OK;
}

BCS_RESULT c_haloreach_cache_file_reader::get_section_buffer(e_cache_file_section section_index, s_cache_file_buffer_info& buffer_info) const
{
...
BCS_RESULT c_haloreach_cache_file_reader::get_buffer(e_cache_file_buffer_index buffer_index, s_cache_file_buffer_info& buffer_info) const
{
	switch (buffer_index)
	{
...
BCS_RESULT c_haloreach_cache_file_reader::get_buffers(s_cache_file_buffers_info& buffers_info) const
{
...
BCS_RESULT c_haloreach_cache_file_reader::associate_cache_cluster(c_cache_cluster& target_cache_cluster)
{
	if (c_haloreach_cache_cluster* haloreach_cache_cluster = dynamic_cast<c_haloreach_cache_cluster*>(&target_cache_cluster))
	{
		return associate_cache_cluster(*haloreach_cache_cluster);
	}

	return BCS_E_FAIL;
}

BCS_RESULT c_haloreach_cache_file_reader::associate_cache_cluster(c_haloreach_cache_cluster& target_cache_cluster)
{
...
BCS_RESULT c_haloreach_cache_file_reader::virtual_address_to_relative_offset(int64_t virtual_address, int32_t& relative_offset) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
...
BCS_RESULT c_haloreach_cache_file_reader::page_offset_to_virtual_address(uint32_t page_offset, int64_t& virtual_address) const
{
...
BCS_RESULT c_haloreach_cache_file_reader::get_blofeld_tag_groups(blofeld::t_tag_group_collection& tag_groups) const
{
	return blofeld::tag_definition_registry_get_tag_groups_by_engine_platform_build(engine_platform_build, tag_groups);
}

BCS_RESULT c_haloreach_cache_file_reader::get_tags_header_relative_offset(int32_t& tags_header_relative_offset) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
...
BCS_RESULT c_haloreach_cache_file_reader::get_string_id_count(int32_t& string_id_count) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		string_id_count = xbox360_cache_file_header.string_id_count;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_string_id_index_offset(int32_t& string_id_index_offset) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		string_id_index_offset = xbox360_cache_file_header.string_id_index_offset;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_string_id_data_offset(int32_t& string_id_data_offset) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		string_id_data_offset = xbox360_cache_file_header.string_id_data_offset;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_string_id_data_count(int32_t& string_id_data_count) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		string_id_data_count = xbox360_cache_file_header.string_id_data_count;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_debug_tag_name_count(int32_t& debug_tag_name_count) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		debug_tag_name_count = xbox360_cache_file_header.debug_tag_name_count;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_debug_tag_name_index_offset(int32_t& debug_tag_name_index_offset) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		debug_tag_name_index_offset = xbox360_cache_file_header.debug_tag_name_index_offset;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_debug_tag_name_data_offset(int32_t& debug_tag_name_data_offset) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		debug_tag_name_data_offset = xbox360_cache_file_header.debug_tag_name_data_offset;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
...
BCS_RESULT c_haloreach_cache_file_reader::get_debug_tag_name_data_size(int32_t& debug_tag_name_data_size) const
{
	switch (engine_platform_build.platform_type)
	{
	case _platform_type_pc_64bit:
	...
	case _platform_type_xbox_360:
	{
		debug_tag_name_data_size = xbox360_cache_file_header.debug_tag_name_data_size;
		return BCS_S_OK;
	}
	break;
	default: return BCS_E_UNSUPPORTED;
